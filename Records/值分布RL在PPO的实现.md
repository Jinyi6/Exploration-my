...
| 方法                                                       | Critic 训练（学什么分布，target 怎么做，loss 用什么）                                                                                                                                                                                                                                       | 策略更新是否“重加权/额外结构”                                                                                                                                                                                                         | Â 计算（GAE 结构是否变，baseline 怎么来）                                                                                                                                                                                                                      |
| -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **PPO（baseline）**                                        | **学习对象：**标量 (V_\phi(s))。<br>**Target：**MC return 或 TD((\lambda)) return。<br>**Loss：**MSE / Huber：(\big(V_\phi(s)-\hat V\big)^2)。                                                                                                                                           | 标准 PPO：clip ratio + entropy +（可选）KL / early stop。                                                                                                                                                                        | **GAE：**(\delta_t=r_t+\gamma V(s_{t+1})-V(s_t))，(\hat A_t=\sum_{l\ge 0}(\gamma\lambda)^l\delta_{t+l})。                                                                                                                                            |
| **Distributional PPO（通用模板）**                             | **学习对象：**分布 (Z_\phi(s))，常见两类：<br>① **quantiles**：输出 ({z_i(s)})；② mixture / categorical。<br>**Target（最基本写法）：**((T^\pi Z)(s_t)\overset{D}{=} r_t+\gamma Z(s_{t+1}))。<br>**Loss：**分布距离 (D\big(Z^{tgt},Z_\phi(s_t)\big))：quantile-Huber / Wasserstein / **energy distance** 等。 | 取决于你怎么从分布拿到学习信号：<br>(a) **不重加权：**actor 仍然 PPO clip；<br>(b) **样本重加权：**用不确定性/形状一致性给每个样本乘权；<br>(c) **风险条件策略：**policy 额外输入 risk 参数。                                                                                          | 三种典型：<br>(1) **均值 GAE：**把 (V(s)) 换成 (\mathbb E[Z(s)])，GAE 结构不变；<br>(2) **风险 GAE：**(V^\rho(s)=\rho(Z(s))) 再做 GAE；<br>(3) **分布优势：**(\delta_t) 由“分布差异”映射成标量（见 DGAE）。                                                                                 |
| **GMAC / IQAC（ICML 2021）**                               | **核心是 critic：**分布表示三类：IQN（IQAC）、IQN + **energy distance**（IQAC-E）、GMM + **energy distance**（GMAC）。<br>**Target：****SR((\lambda)) sample-replacement** 构造多步分布目标（比单步更适合 on-policy）。<br>**Loss：**IQAC 用 quantile-Huber；IQAC-E / GMAC 用 **energy distance**（对样本/混合分布更友好）。      | 通常 **不改 PPO clip**；“新增部分”主要在 critic / target / loss。                                                                                                                                                                     | 仍是 **GAE** 结构；baseline 用 (\mathbb E[Z(s)])（分布均值）替换标量 (V(s))。因此可写成 (\hat A\approx \mathbb E[Z]-)（更准确：**GAE 里用均值 value**）。                                                                                                                          |
| **DPPO（risk-aware quadruped locomotion）**                | **学习对象：**用 **QR-DQN 风格**输出 ({z_i(s)}) 近似 (Z(s))（等权 atoms / quantiles）。<br>**Target：**强调 **multi-step / sample-replacement**（让分布 target 更“厚”、支持点更丰富）。<br>**Loss：**常用 **energy distance**（样本级分布距离，适配 atoms/quantiles）。                                                       | **关键新增：**<br>① **risk-conditioned policy：**(\pi_\theta(a\mid s,\eta))，(\eta) 控制风险偏好；<br>② 用分布 (Z(s)) 通过风险度量 (\rho) 得到标量目标（用于 actor / baseline）：(V^\rho(s)=\rho(Z(s)))，(\rho) 可取 distortion risk measures（CVaR / Wang 等）。 | 通常 **GAE 结构不变**，但 baseline 取 **风险价值**：<br>(\delta_t=r_t+\gamma V^\rho(s_{t+1})-V^\rho(s_t))，(\hat A_t=\sum_{l\ge 0}(\gamma\lambda)^l\delta_{t+l})。<br>（若 (\rho=\mathbb E) 则退化为均值 GAE。）                                                            |
| **NGDRL（Normality-Guided Distributional RL, TMLR 2025）** | 思想：回报分布常接近正态 ⇒ **不用经典 distributional Bellman target**，改成“正态目标分位点”。<br>实现：一个 **variance network** 预测 (\sigma^2(s))，结合均值/return 得到 (\mathcal N(\mu,\sigma^2)) 的目标 quantiles。<br>**Loss：**预测 quantiles vs 正态目标 quantiles 的回归误差（quantile regression / MAE 等）。                  | **核心新增是样本权重 (w(s))：**用“预测分布 quantiles”与“正态诱导 quantiles”的差异得到 (w(s))，再把 (w(s)) 乘到 PPO/TRPO 的 policy loss 上（更信任“形状一致”的样本）。                                                                                                 | advantage 一般仍用 **常规 GAE（均值 baseline）**；区别不在 (\hat A) 定义，而在 **policy loss 被 (w(s)) 重加权**。                                                                                                                                                          |
| **DGAE（Distributional GAE, arXiv 2025）**                 | **学习对象：**仍是 quantile / inverse-CDF 表示 (Z(s))，用分布式 TD（“分布版 TD”）构造 target，quantile-Huber 等训练。                                                                                                                                                                                | PPO 目标形式不变，但优势信号“来源变了”：用分布优势替代标量优势。                                                                                                                                                                                      | **分布优势（关键）：**先用某个 directional distribution metric 把 (Z^{tgt}) vs (Z(s_t)) 映射为标量 (\delta_t^D)，再做 GAE 指数加权：(\hat A_t^D=\sum_{l\ge 0}(\gamma\lambda)^l\delta_{t+l}^D)。<br>（建议把 (\delta_t^D) 明确解释为“分布差异 (\rightarrow) 标量 TD-error”的算子，避免与标准 GAE 的混淆。） |
