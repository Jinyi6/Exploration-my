...
# Risk-sensitive PPO合理的几组参数配置
注意，critic.risk_apply_to和critic.risk_level被弃用！

1. 实现的就是Distributional PPO本身，没有任何额外risk的信息
risk_apply_to = "target1" or “baseline1”
baseline_mode = "neutral"
risl_level = "neutral"

或者 这个也是PPO 本身，可以测一下，确保没问题
risk_apply_to = "none"
baseline_mode = "risk"
risl_level = "averse"


2. 不改变target，只是改变baseline为风险值，这一般不影响太大的性能，因为只会影响方差
risk_apply_to = "baseline1"
baseline_mode = "risk" 
risl_level = "xxx" # averse / seeking / svar_25/50等

3. 改变target，同时去掉baseline。相当于方差很大的risk-sensitive RL
risk_apply_to = "target1"
baseline_mode = "none"
risl_level = "xxx" # averse / seeking / svar_25/50等

4. 改变target，baseline也是需要保持同样的风险模式。合理的risk-sensitive RL
risk_apply_to = "target1"
baseline_mode = "risk"
risl_level = "xxx" # averse / seeking / svar_25/50等

5. 改变target，baseline也是需要保持同样的风险模式。Tail Reweighting (样本重加权)：通过 $w_t$ 对 $y_t$ 进行加权。
risk_apply_to = "baseline1"
